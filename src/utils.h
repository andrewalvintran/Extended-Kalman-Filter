...
void updateFAndQMatrix(KalmanFilter &ekf, const long long previous, const long long current) {
  ...
  double dt_sq = dt * dt;
  double dt_cubed_half = (dt_sq * dt) / 2.0;
  ...
  ekf.Q_ << dt_fourth/4*NOISE_AX_, 0, dt_cubed_half * NOISE_AX_, 0,
            0, dt_fourth/4 * NOISE_AY_, 0, dt_cubed_half * NOISE_AY_,
            dt_cubed_half * NOISE_AX_, 0, dt_sq * NOISE_AX_, 0,
            0, dt_cubed_half * NOISE_AY_, 0, dt_sq * NOISE_AY_;
}
